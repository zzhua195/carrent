...
<%--查询条件结束--%>
    ...
    <script type="text/html" id="userToolBar">
        <div class="layui-btn-container">
            ...
            <button class="layui-btn layui-btn-sm" lay-event="deleteBatchUser">批量删除</button>
        </div>
    </script>
    <%--头工具条结束--%>
...
<script>
    ...
    layui.extend({
        ...
        userTable = table.render({
            ...
            ,id:'userTable'
            ,toolbar: '#userToolBar'
            ,url:'../user/loadUserByPage.action'
            ,page:true
            ,limit:7
            ,limits:[7,14]
            ,cols: [[
        ...
        form.on('submit(*)', function(data){
            ...
            userTable.reload({
                url:'../user/loadUserByPage.action?'+params,
                page:{
                    curr:1
                }
            });
            ...
            switch(obj.event){
                ...
                case 'deleteBatchUser':
                    ...
                    $.each(data,function (index, item) {
                        if(index==0){
                            params+="uids="+item.userid;
                        }else{
                            params+="&uids="+item.userid;
                        }
                    })
                    $.post('../user/deleteBatchUser.action',params,function (resp) {
                        layer.msg(resp.msg);
                        if(resp.code==0){
                            layer.msg(resp.msg);
                            userTable.reload();
        ...
        table.on('tool(userTable)', function(obj){
            ...
            if(obj.event === 'del'){
                layer.confirm('真的删除用户【'+data.loginname+'】吗?', {offset:'50px'},function(index){
                    ...
                    //删除用户
                    $.post('../user/deleteUser.action?id='+data.userid,function (resp) {
                        if(resp.code==0){
                            layer.close(index);
                            layer.msg(resp.msg);
                            userTable.reload();
                ...
                mainIndex = layer.open({
                    ...
                    content:$("#saveOrUpdateDiv"),
                    success:function (layero, index) {
                        url = '../user/updateUser.action?userid='+data.userid;
                        form.val('saveOrUpdateFrm',data);
                    }
                });
            }else if(obj.event === 'selectUserRole'){
                layer.open({
                    type: 1
                    ...
                    ,success:function (layero,index) {
                        roleTableIns = table.render({
                            elem: '#roleTable'
                            ,url:'${ctx}/role/queryRoleByUid.action?uid='+data.userid
                            ,cols: [[
                    ...
                    ,yes: function(index, layero){
                        ...
                        $.post('${ctx}/user/resetUserRole.action',params,function (resp) {
                            if(resp.code<0){
                                layer.msg(resp.msg);
                            }else{
